...
* Wenn das Tutorial Dich auffordert ein Rails-Kommando, das `rails`, `rake` oder `bundle` enthält in das Terminal einzugeben, stelle dem Kommando ein `bundle exec` voran, also z. Bsp. `bundle exec rake db:migrate`.
